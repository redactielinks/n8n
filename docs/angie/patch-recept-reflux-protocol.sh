#!/bin/bash
# /recept herschrijft nu elk binnengekomen recept (of receptidee/lijst
# ingredienten) automatisch volgens het Reflux-Filter-Protocol: een
# maagvriendelijke, Nutri-Score-A-variant met vaste content-structuur
# (titel/ondertitel, teaser, intro, statstabel, ingredienten,
# instructies per apparaat, meal-prep-advies). Daarna opgeslagen onder
# kennisbank/wiki/koken/, gelinkt vanaf de Koken-categoriepagina, met
# een bevestiging terug die aangeeft of het herschrijven is gelukt.
set -euo pipefail
...
for node in wf['nodes']:
    ...
    if nm == 'Handle Obsidian':
        ...
        print("  ~ Handle Obsidian: /recept herschrijft nu via het Reflux-Filter-Protocol")
...
echo ""
echo "Test (kan 1-2 minuten duren, het lokale model herschrijft het hele recept):"
...
echo "  /recept ik heb kip, broccoli en rijst, wat kan ik daarmee maken?"
