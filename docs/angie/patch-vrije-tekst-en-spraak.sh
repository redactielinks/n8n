#!/bin/bash
# Maakt commando's optioneel: tekst zonder / en spraakberichten worden nu
# ook verwerkt. Gemma bepaalt zelf de intentie (wiki/onderzoek/braindump/
# idee/taak/dagboek/notitie) als je geen prefix typt. Spraakberichten
# worden eerst getranscribeerd via een zelf-gehoste Whisper-server op de
# Mac Mini (zie setup-whisper-mac-mini.sh), en daarna op dezelfde manier
# verwerkt als getypte tekst.
# Vereist: setup-whisper-mac-mini.sh is al uitgevoerd op de Mac Mini.
set -euo pipefail
...
const VRIJE_TEKST_INTENTS = ['notitie', 'dagboek', 'idee', 'taak', 'wiki', 'onderzoek', 'braindump'];
if (!cmdMatchRaw && text) {
  try {
    const llmData = await httpRequest({
      ...
      body: {
        ...
        messages: [{
          role: 'system',
          content: 'Bepaal welke actie het beste past bij dit bericht. Antwoord uitsluitend als JSON: {"intent":"wiki|onderzoek|braindump|idee|taak|dagboek|notitie"}. wiki = vraag over eigen kennisbank/aantekeningen. onderzoek = vraag die feitelijke informatie van het internet nodig heeft. braindump = een gedachte, brainwave of idee dat verder uitgewerkt en gecheckt moet worden. idee = kort, simpel idee zonder verdere uitwerking. taak = iets dat gedaan moet worden. dagboek = persoonlijke reflectie of dagverslag. notitie = losse aantekening die nergens anders bij past.'
...
NEW_CONDITIONS = [
    {"id": "obs-vrije-tekst", "leftValue": "={{ ($json.text || $json.message?.text || '').length > 0 ? 'yes' : 'no' }}", "rightValue": "yes", "operator": {"type": "string", "operation": "equals"}},
    {"id": "obs-voice", "leftValue": "={{ $json.message?.voice ? 'yes' : 'no' }}", "rightValue": "yes", "operator": {"type": "string", "operation": "equals"}},
]
...
for node in wf['nodes']:
    ...
    if nm == 'Handle Obsidian':
        ...
        print("  ~ Handle Obsidian: vrije tekst + spraak toegevoegd")
        ...
        for nc in NEW_CONDITIONS:
            key = nc['id']
            if not any(c.get('id') == key for c in conds):
                conds.append(nc)
                print(f"  ~ Is Obsidian Cmd?: {nc['id']} toegevoegd")
            else:
                print(f"  ~ Is Obsidian Cmd?: {nc['id']} al aanwezig")
...
echo ""
echo "Test (zonder commando):"
echo "  Typ gewoon: wat staat er in mijn wiki over de schrijfstijl?"
echo "  Of stuur een spraakbericht met dezelfde vraag."
