#!/bin/bash
# Voeg /wiki toe aan de werkende Obsidian-route: doorzoekt de kennisbank-wiki
# op GitHub en laat Gemma (LM Studio, Mac Mini) een antwoord formuleren.
set -euo pipefail
...
if (cmd === 'notitie' || cmd === 'notities' || cmd === 'note') {
  ...
  replyText = 'Beschikbare commando\'s:\n/notitie /dagboek /idee /taak /taken /wiki /zoek';
}
...
NEW_CONDITIONS = [
    {"id": "obs-wiki", "leftValue": "={{ ($json.text || '').toLowerCase() }}", "rightValue": "/wiki", "operator": {"type": "string", "operation": "startsWith"}},
]
...
for node in wf['nodes']:
    ...
    if nm == 'Handle Obsidian':
        ...
        print("  ~ Handle Obsidian: /wiki toegevoegd")
