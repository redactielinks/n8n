#!/bin/bash
# ============================================================
# Project Angie — Geconsolideerde fix: rubrieken (tickets/garantie/
# verzekeringen) + echte LLM-titels in plaats van tekst-afkapping
# Machine: Raspberry Pi 5 (100.77.5.104)
# ============================================================
# Bugreport: "ik zie nog steeds geen rubrieken. en de beschrijvingen zijn
# waardeloos. dat zouden korte titels moeten zijn die beschrijven waar de
# inhoud over gaat."
#
# Oorzaken:
# 1. Tickets, garantiebewijzen en verzekeringspolissen hadden geen eigen
#    deterministische herkenning (zoals facturen/recepten die al hadden) --
#    ze waren volledig afhankelijk van de LLM-intentclassificatie, die bij
#    lange geextraheerde PDF-tekst kan timeouten en dan altijd op 'notitie'
#    onder Persoonlijk uitkomt in plaats van de juiste rubriek.
# 2. Titels/previews waren tot nu toe een kale afkapping van de ruwe tekst
#    (eerste regel, of eerste 50-60 tekens). Dat beschrijft zelden waar een
#    document over gaat -- een aanhef ("Geachte heer/mevrouw,") of kopregel
#    is geen samenvatting.
#
# Fix:
# - Nieuwe deterministische heuristieken lijktOpTicket/lijktOpGarantie/
#   lijktOpVerzekering (zelfde patroon als lijktOpRecept/lijktOpFactuur),
#   gecontroleerd voor de LLM-classificatie zodat ze nooit op een timeout
#   kunnen stranden.
# - verwerkFinancieelDocument vraagt het LLM nu ook om "titel_kort": een
#   echte korte samenvattende titel, gebruikt als fallback wanneer er geen
#   leverancier herkend is (facturen/administratie/declaratie/tickets/
#   kopen/garantie/verzekeringen).
# - Nieuwe functie genereerKorteTitel() doet hetzelfde voor notities: een
#   echte LLM-samenvatting in plaats van de eerste tekstregel afkappen.
# - De vrije-tekst LLM-classificatie-timeout is verhoogd van 30s naar 45s
#   voor meer marge bij lange teksten.
#
# Dit is een geconsolideerde patch die in 1x de volledige jsCode van de
# "Verwerk Wiki Commando"-node vervangt (zoals het oorspronkelijke
# patch-facturen-bijlagen-en-fiscale-tips.sh deed), in plaats van een
# kleine surgical diff. Daardoor werkt deze patch ongeacht welke van de
# eerdere vervolg-patches (patch-titel-notities-en-facturen.sh,
# patch-factuurherkenning-regex-fix.sh) je al wel of niet hebt gedraaid --
# die twee scripts zijn met deze patch overbodig geworden en uit de repo
# verwijderd.
#
# Vereist alleen dat patch-facturen-bijlagen-en-fiscale-tips.sh al is
# uitgevoerd (checkt op 'verwerkFinancieelDocument').
#
# Voer dit uit OP de Raspberry Pi. Verwijder eerst een eventuele oude lokale
# kopie en download daarna vers:
#   rm -f patch-rubrieken-en-llm-titels.sh
#   curl -fsSL -o patch-rubrieken-en-llm-titels.sh "https://raw.githubusercontent.com/redactielinks/n8n/claude/angie-https-tunnel-foss-hfqqzl/docs/angie/patch-rubrieken-en-llm-titels.sh?t=$(date +%s)"
#   bash patch-rubrieken-en-llm-titels.sh
# ============================================================
set -euo pipefail

SCRIPT_VERSIE="2026-06-19-rubrieken-en-llm-titels-1"
...
return [{ json: { replyText, chatId, text, source } }];""".strip()
...
if 'verwerkFinancieelDocument' not in huidige_js:
...
print("  ~ 'Verwerk Wiki Commando': rubriekherkenning tickets/garantie/verzekeringen + echte LLM-titels (geen tekst-afkapping meer)")
...
echo ""
echo "Test 1 (ticket-tekst zonder /commando moet automatisch op /tickets"
echo "belanden, ook bij een trage of niet-bereikbare LLM):"
echo '  curl -s -X POST http://localhost:5678/webhook/angie-cli -H "Content-Type: application/json" -d '"'"'{"text":"Boarding pass KLM 1234, Amsterdam Schiphol naar Londen Heathrow, gate 14, vertrektijd 14:20, stoel 22A"}'"'"''
echo "Verwacht: 'Tickets opgeslagen: ...' met een herkenbare titel, NIET als"
echo "losse notitie onder Persoonlijk."
echo ""
echo "Test 2 (notitie krijgt nu een echte LLM-titel, geen afgekapte"
echo "tekstregel):"
echo '  curl -s -X POST http://localhost:5678/webhook/angie-cli -H "Content-Type: application/json" -d '"'"'{"text":"/notitie Geachte heer/mevrouw, bijgaand de jaaropgave van uw zorgverzekering over 2025. Bewaar dit goed voor de belastingaangifte."}'"'"''
echo "Verwacht: titel beschrijft de inhoud (bv. iets met jaaropgave/"
echo "zorgverzekering/belastingaangifte), niet de aanhefregel \"Geachte"
echo "heer/mevrouw...\"."
echo "  cat \$(ls -t ~/kennisbank/wiki/persoonlijk/notities/*.md | head -1) | head -3"
