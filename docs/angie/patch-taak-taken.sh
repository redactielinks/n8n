#!/bin/bash
# Voeg /taak en /taken toe aan de werkende Obsidian-route
set -euo pipefail
...
if (cmd === 'notitie' || cmd === 'notities' || cmd === 'note') {
  ...
  replyText = 'Beschikbare commando\'s:\n/notitie /dagboek /idee /taak /taken /zoek';
}
...
NEW_CONDITIONS = [
    {"id": "obs-taak",  "leftValue": "={{ ($json.text || '').toLowerCase() }}", "rightValue": "/taak",  "operator": {"type": "string", "operation": "startsWith"}},
    {"id": "obs-taken", "leftValue": "={{ ($json.text || '').toLowerCase() }}", "rightValue": "/taken", "operator": {"type": "string", "operation": "startsWith"}},
]
...
for node in wf['nodes']:
    ...
    if nm == 'Handle Obsidian':
        ...
        print("  ~ Handle Obsidian: /taak en /taken toegevoegd")
