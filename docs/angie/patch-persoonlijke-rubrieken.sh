#!/bin/bash
# ============================================================
# Project Angie — Nieuwe persoonlijke rubrieken (administratie, declaratie,
# tickets, kopen, garantie, verzekeringen)
# Machine: Raspberry Pi 5 (100.77.5.104)
# ============================================================
# Voegt zes nieuwe simpele opslag-rubrieken toe onder Persoonlijk, naast de
# bestaande notities/dagboek/ideeen/taken/braindumps:
#   /administratie, /declaratie, /tickets, /kopen, /garantie, /verzekeringen
# Elke rubriek werkt als /notitie: tekst wordt direct weggeschreven (geen
# LLM-verrijking nodig) met het juiste label/tag, en gelinkt vanaf
# persoonlijk.md. Typ je geen commando maar gewoon vrije tekst, dan kan de
# LLM-intentieclassificatie nu ook een van deze zes rubrieken herkennen
# (bijv. een gedeeld vliegticket -> /tickets).
#
# Voer dit uit OP de Raspberry Pi. Verwijder eerst een eventuele oude lokale
# kopie en download daarna vers:
#   rm -f patch-persoonlijke-rubrieken.sh
#   curl -fsSL -o patch-persoonlijke-rubrieken.sh "https://raw.githubusercontent.com/redactielinks/n8n/claude/angie-https-tunnel-foss-hfqqzl/docs/angie/patch-persoonlijke-rubrieken.sh?t=$(date +%s)"
#   grep -q "SIMPELE_RUBRIEKEN" patch-persoonlijke-rubrieken.sh && echo "OK: juiste versie gedownload" || echo "FOUT: oude/verkeerde versie"
#   bash patch-persoonlijke-rubrieken.sh
# ============================================================
set -euo pipefail

SCRIPT_VERSIE="2026-06-19-persoonlijke-rubrieken-1"
...
// Eenvoudige opslag-rubrieken onder Persoonlijk: geen LLM-verrijking nodig,
// gewoon de tekst wegschrijven met het juiste label/tag en linken vanaf
// persoonlijk.md, op dezelfde manier als notitie/taak.
...
function slaSimpeleRubriekOp(rubriek, label, content) {
  const dir = path.join(KENNISBANK_WIKI, 'persoonlijk', rubriek);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const filename = fileTs + '-' + rubriek + '.md';
  const lines = ['---', 'tags:', '  - ' + rubriek, '  - ' + source, 'datum: ' + timestamp, 'bron: ' + source, '---', '', content];
  ...
  linkInCategorie('persoonlijk.md', 'Persoonlijk', label + ': ' + preview, 'persoonlijk/' + rubriek + '/' + filename);
  return preview;
}
...
Begin je antwoord direct met de titel als "# Titel", zonder inleidende zin ervoor.`;
  ...
  if (!content) {
    ...
    const preview = slaSimpeleRubriekOp(cmd, SIMPELE_RUBRIEKEN[cmd], content);
    replyText = SIMPELE_RUBRIEKEN[cmd] + ' opgeslagen: "' + preview + '"';
  }
...
if "const source = inp.source || 'telegram';" not in huidige_js:
    raise SystemExit("Verwachte 'source'-toevoeging niet gevonden in de huidige jsCode -- patch afgebroken, vermoedelijk is een eerdere patch nog niet uitgevoerd.")
if 'lijktOpRecept' not in huidige_js:
    raise SystemExit("Verwachte 'lijktOpRecept'-heuristiek (uit patch-recept-detectie-en-cosmetica.sh) niet gevonden -- patch afgebroken, vermoedelijk is die patch nog niet uitgevoerd.")
if 'SIMPELE_RUBRIEKEN' in huidige_js:
    raise SystemExit("Deze patch lijkt al uitgevoerd te zijn ('SIMPELE_RUBRIEKEN' staat er al in) -- niet opnieuw uitvoeren.")

handler['parameters']['jsCode'] = NIEUWE_JS
print("  ~ 'Verwerk Wiki Commando': zes nieuwe rubrieken toegevoegd (administratie, declaratie, tickets, kopen, garantie, verzekeringen)")
...
echo ""
echo "Test (nieuwe rubriek via de CLI-bypass):"
echo '  curl -s -X POST http://localhost:5678/webhook/angie-cli -H "Content-Type: application/json" -d '"'"'{"text":"/tickets KLM vlucht AMS-BCN, 12 juli, boarding 06:40"}'"'"''
echo "Verwacht: 'Tickets opgeslagen: ...' en een nieuw bestand onder ~/kennisbank/wiki/persoonlijk/tickets/, gelinkt vanaf persoonlijk.md."
echo ""
echo "Controleer alle zes rubrieken:"
echo '  for r in administratie declaratie tickets kopen garantie verzekeringen; do curl -s -X POST http://localhost:5678/webhook/angie-cli -H "Content-Type: application/json" -d "{\"text\":\"/$r testbericht voor $r\"}"; echo; done'
echo "  ls ~/kennisbank/wiki/persoonlijk/"
echo "Verwacht: alle zes mappen bestaan, elk met 1 nieuw bestand, en persoonlijk.md heeft zes nieuwe links."
