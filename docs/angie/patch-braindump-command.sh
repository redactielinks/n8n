#!/bin/bash
# Voeg /braindump (alias /park) toe: parkeert een gedachte direct en
# onvoorwaardelijk in de vault, en werkt die daarna uit met een
# verificatie-zoekopdracht via SearXNG, een LLM-beoordeling (klopt het,
# hoe werk je het verder uit, welke varianten zijn er) en een Mermaid-
# mindmap die in Obsidian direct gerenderd wordt.
# Vereist: setup-searxng.sh is al uitgevoerd (patch-onderzoek-command.sh).
# De gedachte zelf wordt altijd opgeslagen, ook als de verdere verwerking
# mislukt — parkeren is de eerste stap, niet afhankelijk van de rest.
set -euo pipefail
...
NEW_CONDITIONS = [
    {"id": "obs-braindump", "leftValue": "={{ ($json.text || '').toLowerCase() }}", "rightValue": "/braindump", "operator": {"type": "string", "operation": "startsWith"}},
    {"id": "obs-park", "leftValue": "={{ ($json.text || '').toLowerCase() }}", "rightValue": "/park", "operator": {"type": "string", "operation": "startsWith"}},
]
...
for node in wf['nodes']:
    ...
    if nm == 'Handle Obsidian':
        ...
        print("  ~ Handle Obsidian: /braindump toegevoegd")
...
echo ""
echo "Test:"
echo "  /braindump wat als ik een cursus geef over twice exceptional ondernemerschap"
echo ""
echo "Resultaat staat in: n8n-obsidian-share/Braindumps/<timestamp>-braindump.md"
echo "Open dat bestand in Obsidian om de mindmap gerenderd te zien."
