#!/bin/bash
# ============================================================
# Project Angie — Obsidian uitfaseren, alles naar de eigen wiki
# Machine: Raspberry Pi 5 (100.77.5.104)
# ============================================================
# /notitie, /dagboek, /idee, /taak, /taken, /braindump en /zoek
# schreven en lazen tot nu toe in de Obsidian-vault
# (/home/node/obsidian-share). Die wordt niet meer gebruikt: alles
# gaat voortaan naar kennisbank/wiki/persoonlijk/, op dezelfde manier
# als /recept al naar kennisbank/wiki/koken/ schrijft, met een link
# vanaf de Persoonlijk-categoriepagina zodat niets een wees-pagina
# wordt. /wiki, /onderzoek en /recept gebruikten de wiki al en zijn
# ongewijzigd.
#
# Voer dit uit OP de Raspberry Pi:
#   curl -fsSL -o patch-stop-obsidian-gebruik-wiki.sh https://raw.githubusercontent.com/redactielinks/n8n/claude/angie-https-tunnel-foss-hfqqzl/docs/angie/patch-stop-obsidian-gebruik-wiki.sh
#   bash patch-stop-obsidian-gebruik-wiki.sh
# ============================================================
set -euo pipefail
...
Begin je antwoord direct met de titel als "# Titel", zonder inleidende zin ervoor.`;
    ...
    try {
      ...
      if (raw) {
        ...
        if (titleMatch) titel = titleMatch[1].substring(0, 60).trim();
        herschreven = true;
      }
    } catch (e) {
    ...
    fmLines.push('  - telegram', 'datum: ' + timestamp, 'bron: telegram', '---');
...
for node in wf['nodes']:
    ...
    if nm == 'Handle Obsidian':
        ...
        print("  ~ Handle Obsidian: schrijft niet meer naar Obsidian, alles naar kennisbank/wiki/persoonlijk")
...
echo ""
echo "Test (alle commando's schrijven nu naar kennisbank/wiki/persoonlijk, niet meer naar Obsidian):"
echo "  /notitie test van de nieuwe wiki-opslag"
echo "  /dagboek vandaag ging het goed"
echo "  /idee een cursus over twice exceptional ondernemerschap"
echo "  /taak boodschappen doen"
echo "  /taken"
echo "  /zoek test"
echo ""
echo "Controleer op de wiki-site (https://<jouw-tailscale-naam>/) onder Persoonlijk."
