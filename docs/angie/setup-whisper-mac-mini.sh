...
# ── 3. Server-script wegschrijven ─────────────────────────────
# Ingebed als heredoc (niet gekopieerd vanaf schijf), zodat dit ook werkt
# wanneer dit script via "bash <(curl ...)" wordt uitgevoerd, waarbij er
# geen whisper_server.py naast dit script op schijf staat.
cat > "$APP_DIR/whisper_server.py" << 'PYEOF'
import os
import tempfile
